...
run.until.change.prosse.multi <- function(lineages, info, k, b, mu, lambda, char, trait.q, trait.p, ntrait, trait.k, trait.r, trait.name, max.t, n.species) {
    ...
    repeat {
      ...
      r <- (b + mu) * n.extant + sum(lx) + r.tot
      dt <- 1/(r*k)
      
      if ( runif(1) < p.change ) {
        if ( runif(1) < b * n.extant / r ) {# speciation initiation
            i <- sample(n.extant, 1)
            info <- speciate.prosse.multi(info, lineages[i], trait.name)
            lineages <- c(lineages[-i], c(-1,0) + nrow(info))
        } else if ( runif(1) < mu * n.extant / r ) { # extinction
            i <- sample(n.extant, 1)
            info$extinct[lineages[i]] <- TRUE
            lineages <- lineages[-i]
        } else if ( runif(1) < sum(lx) / r ) { # speciation completion due to incompatibility
            i <- sample(n.extant, 1, prob=lx)
            info$species[lineages[i]] <- n.species <- n.species + 1
        } else {
            for (z in 1:ntrait) {
                if ( runif(1) < sum(r.n[[z]]) / r.tot || z == ntrait) {# transition in trait j
