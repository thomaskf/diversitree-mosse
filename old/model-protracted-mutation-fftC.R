...
make.branches.protracted.mutation.fftC <- function(control) {
  check.fftC()
  ...
  f.hi <- make.pde.protracted.mutation.fftC(nx*r, dx/r, dt.max, 3L, flags)
  f.lo <- make.pde.protracted.mutation.fftC(nx,   dx,   dt.max, 3L, flags)
  combine.branches.protracted(f.hi, f.lo, control)
}

make.pde.protracted.mutation.fftC <- function(nx, dx, dt.max, nd, flags) {
  ptr <- .Call(r_make_protracted_mutation_fft, as.integer(nx), as.numeric(dx),
  ...
  function(y, len, pars, t0, star, dt=dt.max) {
    ...
    ans <- .Call(r_do_integrate_protracted_mutation,
