...
## 1: make
make.protracted.representative <- function (tree,control=list(compiled=FALSE,backend="gslode",condition.surv=T)) {
	cache <- make.cache.protracted.representative(tree,control)
	all.branches <- make.all.branches.protracted.representative(cache,cache$control)
...
make.cache.protracted.representative <- function (tree,control) {
	## 1: tree
...
initial.conditions.protracted.representative <- function(init, pars, t)
  c(init[[1]][1],init[[1]][2],
...
make.all.branches.protracted.representative <- function (cache,control) {
	branches <- make.branches.protracted.representative(cache$info,control)
	function (pars) {
		cache$y <- rep(list(c(0,1,1,0,0,0)),cache$n.tip)
		all.branches.matrix.protracted.representative(pars,cache,initial.conditions=initial.conditions.protracted.representative,branches)
	}
}

make.branches.protracted.representative <- function(info,control) {
	eps <- control$eps
	...
	#ode <- list(make.ode(info[[1]],control),make.ode(info[[2]],control))
	#branches <- function(y, len, pars, t0, star) {ode[[star]](y, c(t0, t0+len), pars)} #function to integrate ODE along a branch
  	make.branches.comp.representative(branches, eps)
}

all.branches.matrix.protracted.representative <- function (pars,cache,initial.conditions,branches) {
    ...
    for ( x in 1:n.tip ) {
    	branch.init[[x]] <- y[[x]]
    	#ans <- branches(branch.init[[x]], len[x], pars, 0, 1)
...
make.branches.comp.representative<- function(branches, eps=0) {
    function(y, len, pars, t0, star) {
      #ret <- branches(y, len, pars, t0, star)
      ...
      if ( all(ret[comp.idx] >= eps) && all(ret[comp.idx] <=1) ) {
        ...
        #ret1 <- Recall(y, len1, pars, t0, star)
        ret1 <- Recall(y, len1, pars, t0)
        #ret2 <- Recall(ret1[[2]][,n1], len2, pars, t0 + ti, star)
...
# make.info.protracted.representative <- function (phy) {
	# list(
	# list(name="protractedreptip",
	     # derivs=derivs.protractedreptip,
		 # np=3L,
		 # ny=5L,
		 # k=2L,
		 # idx.e=1:2,
		 ...
		 # ml.default="subplex",
		 # mcmc.lowerzero=TRUE),
	# list(name="protractedrep",
	     # derivs=derivs.protractedrep,
		 # np=3L,
		 # ny=6L,
		 # k=2L,
		 # idx.e=1:2,
...
derivs.protractedreptip <- function (t,y,pars) {
			Ei <- y[1]
			Er <- y[2]
			Dr <- y[3]
			Di <- y[4]
			...
			c(-(b+mu)*Ei + b*Ei*Ei + mu,
			  ...
			  -(b+mu)*Dr + 2*b*Dr*Ei + lambda*(Er-Ei),
			  -(b+mu+lambda)*Di + 2*b*Di*Er + lambda*(Er-Ei+Dr),
			  -(b+mu+lambda)*Dir + 2*b*Dir*Ei + lambda*(Er-Ei+Dr))
}

derivs.protractedrep <- function (t,y,pars) {
			Ei <- y[1]
			Er <- y[2]
			D <- y[3]
			Dr <- y[4]
