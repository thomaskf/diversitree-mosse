...
## 1: make
make.protracted.sampling <- function (tree,control=list(compiled=FALSE,backend="deSolve",root=ROOT.OBS)) {
	cache <- make.cache.protracted.sampling(tree,control)
	all.branches <- make.all.branches.protracted.sampling(cache,cache$control)
	rootfunc <- function (res, pars, condition.surv=TRUE, root, root.p=NULL) {
  		vals <- res$vals
  		idx <- res$idx
  		n <- pars[4]
  		lq <- res$lq
  		...
  		if ( condition.surv ) { #condition the likelihood on that the survivial of the root
    		...
    		d.root[-1] <- d.root[-1] / (b * (1 - er.root) * (1 - vals[1]))
  		}
	...
	ll <- function (pars, condition.surv=TRUE) {
		...
		rootfunc(ans, pars, condition.surv, root)
	}
...
initial.tip.protracted.sampling <- function (cache) {
	...
	init <- function (tip,sampling.f,tmp0,tmp1,tmp2) {
		...
		list(y=c(0,1,0,0,0,0,1-sampling.f,numeric(length(sampling.f)),Drsp),idx=idx)
	}
...
all.branches.matrix.protracted.sampling <- function (pars,cache,initial.conditions,branches) {
	...
	y.in <- initial.conditions(branch.base[children[root,]], pars)
   	...
   	ans <- branches(branch.init[[root]]$y, len[i], pars, depth[i], branch.init[[root]]$idx)
    lq[root] <- ans[[1]]
    branch.base[[root]] <- list(y=ans[[2]],idx=branch.init[[root]]$idx)
	list(init=branch.init, base=branch.base, lq=lq, vals=y.in$y, idx=y.in$idx, er.root=branch.base[children[root,1]]$y[2])
}   	
