...
initial.tip.protracted.sampling <- function (cache) {
	...
	init <- function (tip,sampling.f,tmp0,tmp1,tmp2) {
		...
		list(y=c(numeric(6),1-sampling.f,numeric(length(sampling.f)),Drsp),idx=idx)
	}
...
derivs.protracted.sampling <- function (t,y,pars) {
			...
			res[1:6] <- c(-(b+mu)*Ei + b*Ei*Ei + mu,
			  ...
			  -(b+mu+lambda)*Di + 2*b*Di*Ei + lambda/(N+1)*Dr + lambda/(N+1)*tmp,
			  -(b+mu+lambda*N/(N+1))*Dr + 2*b*Dr*Ei + lambda/(N+1)*tmp,
			  -(b+mu+lambda)*Disp0 + 2*b*Disp0*Ei + lambda/(N+1)*Dr + lambda/(N+1)*tmp,
			  -(b+mu+lambda)*Disp1 + 2*b*Disp1*Er + lambda/(N+1)*Dr + lambda/(N+1)*tmp)
			if (n>0) {
				res[7:(6+n)] <- -(b+mu)*Esp + b*Esp*Esp + mu
				res[(7+n):(6+n+n)] <- -(b+mu+lambda)*Disp + 2*b*Disp*Esp + lambda/(N+1)*Dr + lambda/(N+1)*tmp
			}
