...
## 1: make
make.protracted <- function (tree,control=list(setbound)) {
...
make.cache.protracted <- function (tree,control) {
	## 1: tree
  	...
  	## 2: Control structure
	cache <- make.cache.tree.protracted(tree,control)
...
initial.conditions.protracted <- function(init, pars, t)
...
make.cache.tree.protracted <- function(tree,control) {
  ...
  if (control$setbound) {
  	tip.length <- edge.length[sapply(1:n.tip,function (i) which(edge[,2]==i))]
  	species[which(tip.length==min(tip.length))] <- species[which(tip.length==min(tip.length))[1]]
  }
